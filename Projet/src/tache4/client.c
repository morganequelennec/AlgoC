...
int envoie_recois_message(int socketfd, char *data, char *code)
{
    char message[100];
    char *token;
    // Pas d'input nécessaire pour le nom de client
    if (strcmp(code, "nom") == 0)
    {
        gethostname(message, 100);
    }
    else
    {
    ...
    strcat(data, code);
    strcat(data, "\",\n\t\"valeurs\" : [ ");
    // traitement différent du message et du nom : les espaces ne sont pas des séparateurs
    if (strcmp(code, "message") != 0 && strcmp(code, "nom") != 0)
    {
        /* traitement 1er élément */
...
int main(int argc, char **argv)
{
   ...
   */
    ...
    while (done != 1)
    {
        ...
        {
            envoie_recois_message(socketfd, data, "nom");
            done = 1;
        }
        else if (strcmp(fonction, "calcul") == 0)
        {
            printf("Saisir une operation sous le format : [Operateur ou fonctionnalité (moyenne, min, max, ecart) / nombre1 / nombre 2 ... nombre N]");
